<script>
	...
	import { onMount, getContext, afterUpdate, tick } from 'svelte';
		...
		<div class="w-full sm:max-w-md px-10 min-h-screen flex flex-col text-center">
			{#if ($config?.trusted_header_auth ?? false) || $config?.auth === false}
				<div class="my-auto pb-10 w-full">
					<h1 class="flex items-center justify-center gap-3 text-xl sm:text-2xl text-center font-bold dark:text-gray-200">
						<span>{$i18n.t('Signing in')} {$i18n.t('to')} {$WEBUI_NAME}</span>
						<Spinner />
					</h1>
				</div>
			{:else}
				<div class="my-auto pb-10 w-full dark:text-gray-100">
					...
					>
						<h1 id="auth-form-title" class="text-2xl font-bold">
							{mode === 'signin' ? $i18n.t('Sign in') : $i18n.t('Sign up')}
							{$i18n.t('to')} {$WEBUI_NAME}
						</h1>
						...
						<div class="flex flex-col mt-4">
							{#if mode === 'signup'}
								<div>
									<label for="fullName" class="text-sm font-semibold text-left mb-1">
										{$i18n.t('Name')}
									</label>
									<input
										id="fullName"
							...
							<div class="mb-2">
								<label for="email" class="text-sm font-semibold text-left mb-1">
									{$i18n.t('Email')}
								</label>
								<input
									id="email"
